...
final class OpeningStockRules
{
    public static function assertDraft(OpeningStock $document): void
    {
        if ($document->status !== OpeningStockStatus::Draft) {
            abort(422, 'Only draft opening stock documents can be modified.', [
                'X-Error-Code' => 'OPENING_STOCK_NOT_DRAFT',
            ]);
        }
    }

    public static function assertPostable(OpeningStock $document): void
    {
        self::assertDraft($document);

        if ($document->lines()->count() === 0) {
            abort(422, 'Cannot post an opening stock document without lines.', [
                'X-Error-Code' => 'OPENING_STOCK_NO_LINES',
            ]);
        }
    }
