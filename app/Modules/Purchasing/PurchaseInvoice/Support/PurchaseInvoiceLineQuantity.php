...
final class PurchaseInvoiceLineQuantity
{
    /**
    ...
    {
        if ($quantity <= 0) {
            abort(422, 'Purchase invoice line quantity must be greater than zero.', [
                'X-Error-Code' => 'PURCHASE_INVOICE_LINE_INVALID_QUANTITY',
            ]);
        }
        ...
        if (! $itemUom) {
            abort(422, 'Item UOM does not belong to this item.', [
                'X-Error-Code' => 'PURCHASE_INVOICE_ITEM_UOM_MISMATCH',
            ]);
        }
        ...
        if (bccomp($baseQuantity, '0', 6) <= 0) {
            abort(422, 'Purchase invoice line base quantity must be greater than zero.', [
                'X-Error-Code' => 'PURCHASE_INVOICE_LINE_INVALID_BASE_QUANTITY',
            ]);
        }
